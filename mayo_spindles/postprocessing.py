...
class Metric:
    ...
    def detection_get_binary_labels_and_confidences(self, y_true: np.ndarray, y_pred: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        # y is unbatched - shape [30, 3]
        
        labels = y_true[:, 0]
        confidences = y_pred[:, 0]
        return labels, confidences
    
    def segmentation_get_binary_labels_and_confidences(self, y_true: np.ndarray, y_pred: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        # y is unbatched - shape [750030, 3]
        
        labels = y_true[:, 0]
        confidences = y_pred[:, 0]
        return labels, confidences
    
    def detection_compute_tp_fp_fn(self, seq_len: int, y_true: np.ndarray, y_pred: np.ndarray, confidence_threshold=0.5) -> tuple[int, int, int]:  
        # y is unbatched - shape [30, 3]      
        y_true = Evaluator.detections_to_intervals(y_true, seq_len)
        y_pred = Evaluator.detections_to_intervals(y_pred, seq_len, confidence_threshold=confidence_threshold)
...
class AUROCAndAP(Metric):
    @property
    def labels_and_confidences_getter(self):
        raise NotImplementedError("This method is not implemented yet")
    ...
    def __call__(self, b_y_true, b_y_pred):
        for y_true, y_pred, _cls in zip(b_y_true[self.source_key], b_y_pred[self.source_key], b_y_true['class']):
            y_true, confidences = self.labels_and_confidences_getter(y_true, y_pred)
            
            _cls = Evaluator.CLASSES_INV[_cls[0]]
            self._y_true[_cls].extend(y_true)
            self._confidences[_cls].extend(confidences)
    ...
    def results(self):
        ...
        for cls in self._y_true:
            y_true = np.array(self._y_true[cls])
            confidences = np.array(self._confidences[cls])
            
            if len(np.unique(y_true)) > 1:  # Ensure there are both positive and negative samples
                auroc[cls] = roc_auc_score(y_true, confidences)
                average_precision[cls] = average_precision_score(y_true, confidences)
            else:
                auroc[cls] = np.nan
                average_precision[cls] = np.nan
        ...
        # Calculate macro-average
        macro_average = pd.DataFrame({
            'auroc': np.nanmean(list(auroc.values())),
            'average_precision': np.nanmean(list(average_precision.values()))
        }, index=['macro-average'], dtype=np.float32)
    
        # Calculate micro-average
        all_y_true = np.concatenate([self._y_true[cls] for cls in self._y_true])
        all_confidences = np.concatenate([self._confidences[cls] for cls in self._confidences])
        
        if len(np.unique(all_y_true)) > 1:  # Ensure there are both positive and negative samples
            micro_auroc = roc_auc_score(all_y_true, all_confidences)
            micro_average_precision = average_precision_score(all_y_true, all_confidences)
            
            # Calculate thresholds for micro-average
            ...
            micro_auroc = np.nan
            micro_average_precision = np.nan
            curves['micro-average'] = {
        ...
        return [raw, micro_macro_average, curves]
    ...
    def __str__(self):
        return f"{self.name} ({len(self._y_true)} batches)"
    
    def __len__(self):
        return len(self._y_true[next(iter(self._y_true.keys()))])
    
    
class DetectionAUROCAndAP(AUROCAndAP):    
    @property
    def labels_and_confidences_getter(self):
        return self.detection_get_binary_labels_and_confidences
...
class SegmentationAUROCAndAP(AUROCAndAP):
    @property
    def labels_and_confidences_getter(self):
        return self.segmentation_get_binary_labels_and_confidences
...
class Evaluator:
    ...
    @staticmethod
    def apply_segmentation_boost(y_pred: dict[str, np.ndarray], truncate_negatives=False) -> dict[str, np.ndarray]:
        # Take the 'segmentation' output and boost the confidence of the detections by median of the corresponding segmentation
        # Assume batched input
        y_pred = deepcopy(y_pred)
        seg = y_pred['segmentation']
        
        # Reshape the segmentation to [B, num_segments, -1]
        num_segments = y_pred['detection'].shape[1]
        seg = seg.reshape(seg.shape[0], num_segments, -1)
        
        # Calculate median of each segment
        confidence_boost = np.median(seg, axis=-1)
        
        if truncate_negatives:
            # If the median is negative, set it to 0
            confidence_boost[confidence_boost < 0] = 00
        
        # Boost the confidence of the detections
        y_pred['detection'][:, :, 0] += confidence_boost
        ...
        if should_preprocess_predictions:
            ...
            if 'segmentation' in y_pred:
                ...
                for el in y_pred['detection']:
                    segmentations.append(Evaluator.detections_to_segmentation(el, y_true['segmentation'].shape[-2], confidence_threshold=0.0))
                    
                y_pred['segmentation'] = np.stack(segmentations)
                
            # Apply smoothing to segmentation
            y_pred['segmentation'] = Evaluator.segmentation_smoothing(y_pred['segmentation'])
